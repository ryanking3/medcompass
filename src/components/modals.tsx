...
import type { CreatedTopic, DocumentKind, Notify, StudyCourse, StudyDocument, StudyTopic } from "./types";

export function UploadModal({ onClose, notify, onUploadComplete, topics, selectedTopicId }: { onClose: () => void; notify: Notify; onUploadComplete: (document: StudyDocument) => void; topics: StudyTopic[]; selectedTopicId: string | null }) {
  ...
  const startUpload = async () => {
    ...
    const response = await fetch("/api/documents", {
      ...
      body: JSON.stringify({ originalFilename: file.name, title, kind, storagePath, fileSize: file.size, topicId: topicId || null }),
    });
  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="upload-modal" role="dialog" aria-modal="true" aria-labelledby="upload-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <p className="eyebrow">Add a private source</p><h2 id="upload-title">Bring a textbook or lecture into your workspace</h2>
