...
export function WorkspaceHome({ courses, documents, notes, flashcards, planBlocks, exams, onCreateTopic, onOpenTopic, onOpenLibrary, onOpenPlanner }: WorkspaceHomeProps) {
  ...
  const todayBlocks = planBlocks.filter((block) => block.startsOn === todayString() && block.status === "planned");
  ...
  return <div className="workspace-home">
    ...
    <section className="today-study"><div><p className="eyebrow">Today&apos;s study</p><h2>{todayBlocks.length ? `${minutesLabel(todayMinutes)} planned` : "No blocks planned for today"}</h2><p>{todayBlocks.length ? "Your planned study blocks for tonight." : "Open the planner to turn upcoming exams into study blocks."}</p></div>{todayBlocks.length ? <div className="today-blocks">{todayBlocks.slice(0, 3).map((block) => <button key={block.id} onClick={onOpenPlanner}><strong>{block.title}</strong><small>{minutesLabel(block.durationMinutes)} · {examById.get(block.examId)?.title ?? "Exam"}</small></button>)}</div> : <button className="button ghost" onClick={onOpenPlanner}>Open planner</button>}</section>
