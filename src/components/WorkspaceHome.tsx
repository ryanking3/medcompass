...
import type { StudyCourse, StudyDocument, StudyExam, StudyFlashcard, StudyNote, StudyPlanBlock, StudyPracticeExam, StudyTopic } from "./types";
...
type WorkspaceHomeProps = {
  ...
  exams: StudyExam[];
  practiceExams: StudyPracticeExam[];
  onCreateTopic: () => void;
...
export function WorkspaceHome({ courses, documents, notes, flashcards, planBlocks, exams, practiceExams, onCreateTopic, onOpenTopic, onOpenLibrary, onOpenAtlas, onOpenPlanner, onOpenPractice }: WorkspaceHomeProps) {
  ...
  const totalPracticeAttempts = practiceExams.reduce((total, exam) => total + exam.attempts.length, 0);
  const latestPracticeAttempt = practiceExams.flatMap((exam) => exam.attempts.map((attempt) => ({ ...attempt, paperTitle: exam.title }))).sort((first, second) => new Date(second.completedAt).getTime() - new Date(first.completedAt).getTime())[0] ?? null;
  ...
  return <div className="workspace-home">
    ...
    <section className="practice-pulse">
      <div>
        <p className="eyebrow">Practice pulse</p>
        <h2>{latestPracticeAttempt ? `${latestPracticeAttempt.answeredCount}/${latestPracticeAttempt.questionCount} answered last sit` : practiceExams.length ? `${practiceExams.length} generated ${practiceExams.length === 1 ? "paper" : "papers"}` : "No practice papers yet"}</h2>
        <p>{latestPracticeAttempt ? `${latestPracticeAttempt.paperTitle} · completed ${formatShortDate(latestPracticeAttempt.completedAt.slice(0, 10))}` : practiceExams.length ? "Open Practice to sit one of your generated papers." : "Generate a paper from an exam once the planner has your scope."}</p>
      </div>
      <dl>
        <div><dt>Papers</dt><dd>{practiceExams.length}</dd></div>
        <div><dt>Attempts</dt><dd>{totalPracticeAttempts}</dd></div>
      </dl>
      <button className="button ghost" onClick={onOpenPractice}>{practiceExams.length ? "Open practice →" : "Generate practice →"}</button>
    </section>
