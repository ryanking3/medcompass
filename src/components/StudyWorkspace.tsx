...
import type { AppView, ChatLaunchContext, CreatedTopic, StudyAvailabilityRule, StudyCourse, StudyDocument, StudyExam, StudyFlashcard, StudyNote, StudyPlanBlock, StudyPracticeExam, StudyTopic } from "@/components/types";
...
export function StudyWorkspace({ userId, email, fullName, initialDocuments, initialCourses, initialNotes, initialFlashcards, initialExams, initialAvailability, initialPlanBlocks, initialPracticeExams }: StudyWorkspaceProps) {
  ...
  const openChatWithContext = (context: ChatLaunchContext) => {
    setChatLaunchContext(context);
    setView("chat");
  };
  ...
  return (
    <main className={sidebarCollapsed ? "app-shell app-shell-sidebar-collapsed" : "app-shell"}>
      ...
      <section className="content-area">
        ...
        {view === "chat" && <AiChatPage courses={courses} documents={documents} launchContext={chatLaunchContext} onLaunchContextConsumed={() => setChatLaunchContext(null)} onNoteCreated={handleNoteCreated} onCardCreated={handleCardCreated} onOpenNotesForTopic={openNotesForTopic} onOpenCardsForTopic={openCardsForTopic} />}
        ...
        {view === "practice" && <PracticeExams exams={exams} generatedExams={generatedPracticeExams} onGeneratedExamCreated={(exam) => setGeneratedPracticeExams((current) => [exam, ...current])} onAttemptSaved={handlePracticeAttemptSaved} onStartTimer={startTimer} />}
        {view === "timer" && <StudyTimer timer={activeTimer} onStart={startTimer} onPauseResume={pauseResumeTimer} onClear={() => setActiveTimer(null)} />}
        {view === "dashboard" && (selectedTopic ? <TopicDashboard topic={selectedTopic} course={courses.find((course) => course.id === selectedCourseId) ?? null} documents={documents} notes={notes} flashcards={flashcards} exams={exams} planBlocks={planBlocks} onOpenDocument={openDocument} onOpenCards={() => setView("cards")} onOpenNotes={() => setView("notes")} onOpenUpload={() => setUploadOpen(true)} onOpenPlanner={() => setView("planner")} /> : <WorkspaceHome courses={courses} documents={documents} notes={notes} flashcards={flashcards} planBlocks={planBlocks} exams={exams} practiceExams={generatedPracticeExams} onCreateTopic={() => setTopicOpen(true)} onOpenTopic={selectTopic} onOpenLibrary={() => setView("library")} onOpenAtlas={() => setView("atlas")} onOpenPlanner={() => setView("planner")} onOpenPractice={() => setView("practice")} />)}
